defmodule ElxlogTest do
  ...
  test "alpha_conv" do
    clause = [:clause,[:pred,[:fact,:N,:A]],[[:builtin,[:is,:N1,[:formula,[:-,:N,1]]]],[:pred,[:fact,:N1,:A1]],[:builtin,[:is,:A,[:formula,[:*,:N,:A1]]]]]]
    assert Prove.alpha_conv(clause,1) == [:clause, [:pred, [:fact, {:N, 1}, {:A, 1}]], [[:builtin, [:is, {:N1, 1}, [:formula, [:-, {:N,
 1}, 1]]]], [:pred, [:fact, {:N1, 1}, {:A1, 1}]], [:builtin, [:is, {:A, 1}, [:formula, [:*, {:N, 1}, {:A1,
 1}]]]]]]
    clause1= [:clause,[:pred,[:append,[:X|:Ls],:Ys,[:X|:Zs]]],[[:pred,[:append,:Ls,:Ys,:Zs]]]]
    assert Prove.alpha_conv(clause1,2) == [:clause, [:pred, [:append, [{:X, 2} | {:Ls, 2}], {:Ys, 2}, [{:X, 2} |
{:Zs, 2}]]], [[:pred, [:append, {:Ls, 2}, {:Ys, 2}, {:Zs, 2}]]]]
  end
  ...
  test "unify" do
    ...
    assert Prove.unify([:pred,[:foo,1,2]],[:pred,[:foo,{:X,1},{:Y,1}]],[]) == [[{:Y, 1}, 2], [{:X, 1}, 1]]
  end
